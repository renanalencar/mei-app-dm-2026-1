# Barema de Avaliação - Entrega Final do Projeto

**Valor Total:** 3,0 pontos
**Número de Equipes:** 11

Este barema foi estruturado para avaliar aspectos cruciais do desenvolvimento de software, focando na entrega de funcionalidades, qualidade técnica e experiência do usuário.

## 📊 Critérios de Avaliação

| Critério | Descrição | Pontuação Máxima |
| :--- | :--- | :---: |
| **1. Atendimento aos Requisitos** | Implementação de todas as funcionalidades propostas. O aplicativo cumpre seu propósito e resolve o problema principal estabelecido para o projeto? | **1,0** |
| **2. Qualidade Técnica e Código** | Organização do código, estruturação do projeto, uso de boas práticas (clean code), modularização e ausência de *code smells* ou "gambiarras" evidentes. | **0,9** |
| **3. Interface e Experiência (UI/UX)**| Design amigável, estética adequada, facilidade de uso, responsividade, acessibilidade e feedback visual adequado para as ações do usuário. | **0,7** |
| **4. Estabilidade e Execução** | O projeto pode ser executado sem erros críticos (crashes)? A integração entre front-end e back-end (se aplicável) funciona corretamente? | **0,4** |
| **TOTAL** | | **3,0** |

---

## 📝 Planilha de Lançamento de Notas

Utilize a tabela abaixo para computar e consolidar a nota de cada uma das 11 equipes.

| Equipe | Repositório | M1 (1,0) | M2 (1,0) | M3 (1,0) | Requisitos (1,0) | Código (0,9) | UI/UX (0,7) | Estabilidade (0,4) | **Nota Final (6,0)** | Observações |
| :--- | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :---: | :--- |
| **01** | [GitHub](https://github.com/DavidaMendes/Public_Procurement_Opportunities) |1,0 |1,0 |1,0 |1,0 |0,9 |0,7 |0,4 | **6,0** | |
| **02** | [GitHub](https://github.com/ibiapleo/app-painel-MEI-PNCP) |1,0 |1,0 |1,0 |1,0 |0,9 |0,7 |0,4 | **6,0** | |
| **03** | [GitHub](https://github.com/gabrielfariaxs/Eng.-Dados) |1,0 |1,0 |1,0 |0,5 |0,6 |0,7 |0,4 | **5,2** | Grupo não entregou slides de apresentação, vídeo demonstrativo nem APK da aplicação. |
| **04** | [GitHub](https://github.com/brokendeveloper/projeto-integrador) |1,0 |1,0 |1,0 |1,0 |0,9 |0,7 |0,4 | **6,0** | |
| **05** | [GitHub](https://github.com/Debburiti/noMEI) |1,0 |1,0 |1,0 |1,0 |0,9 |0,7 |0,4 | **6,0** | |
| **07** | [GitHub](https://github.com/henryzera/front-projeto-integrador) |1,0 |1,0 |1,0 |0,8 |0,8 |0,6 |0,3 | **5,5** | Grupo não entregou slides de apresentação nem vídeo demonstrativo. |
| **08** |   [GitHub](https://github.com/lblima038/LicitAcesso) |1,0 |1,0 |1,0 |1,0 |0,9 |0,7 |0,4 | **6,0** | |
| **09** | [GitHub](https://github.com/Arthur5502/projeto-integrador-front) |1,0 |1,0 |1,0 |0,9 |0,9 |0,7 |0,4 | **5,9** | Grupo não entregou vídeo demonstrativo. |
| **10** | [GitHub](https://github.com/VyNas07/Licitei) |1,0 |1,0 |1,0 |1,0 |0,9 |0,7 |0,4 | **6,0** | |
| **11** | [GitHub](https://github.com/Isaac-Daniel-A-D/Editais-Front) |1,0 |1,0 |1,0 |0,8 |0,9 |0,7 |0,4 | **5,7** | Grupo não entregou slides de apresentação nem vídeo demonstrativo. |

---

## 💡 Dicas para Correção
- **0% a 25% do critério:** Não entregou ou entregou algo muito incipiente/com problemas graves.
- **50% do critério:** Entregou parcialmente, funcional, mas com ressalvas significativas.
- **75% a 80% do critério:** Atendeu bem ao critério, faltando apenas refinamento.
- **100% do critério:** Entrega excelente, seguindo o padrão esperado para aquele item.
